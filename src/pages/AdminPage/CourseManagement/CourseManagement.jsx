...
const handleSearch = () => {
  ...
  let keySearch = document.getElementById("keySearch").value.trim().toLowerCase(); 
  ...
  if (keySearch !== "") {
    if (isMaKhoaHoc) {
      ...
      newList = cloneList.filter((item) => item.tenKhoaHoc.toLowerCase().includes(keySearch));
    }
  } else {
  ...
  const columns = [
    ...
    
    {
      ...
      render: (_, record) => (
        ...
        {/* SỬA */}
          <Button onClick={() => {UpdateConfirm(record)}}>
    ...
    <div className='admin__courseMgt'>
      <div className='courseMgt__content'>
        ...
        <div className='courseMgtCont__searchBar my-6 flex md:block'>
          <input id='keySearch' type='search' placeholder='Nhập mã hoặc tên khoá học' className='searchIn__style h-10 w-full px-2 rounded' />
          <ButtonStyled onClick={handleSearch} className='w-28 ml-4 md:mt-2 md:ml-0'>Tìm kiếm</ButtonStyled>
        </div>
